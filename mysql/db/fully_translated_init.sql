...
CREATE TABLE IF NOT EXISTS `aankoop` (
  ...
  PRIMARY KEY (`customer_code`,`article_code`)
...
INSERT INTO `aankoop` (`customer_code`, `article_code`, `amount`) VALUES
(100, 35, 6),
(100, 74, 10),
(101, 384, 5),
(101, 89, 15),
...
--
-- Tabelstructuur voor tabel `aankoopregel`
--

CREATE TABLE IF NOT EXISTS `aankoopregel` (
  `aankoopnummer` int(8) NOT NULL,
  `article_code` int(4) NOT NULL,
  `amount` int(6) NOT NULL,
  `aankoop_prijs` decimal(8,2) NOT NULL,
	Constraint  pk_aankoopregel   primary key	(aankoopnummer,article_code)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `aankoopregel` (`aankoopnummer`, `article_code`, `amount`, `aankoop_prijs`) VALUES
(1, 1, 23, 19.50),
(1, 2, 2, 22.50),
(1, 35, 60, 1.00),
(1, 74, 90, 1.80),
...
CREATE TABLE IF NOT EXISTS `aankopen` (
  `aankoopnummer` int(8) NOT NULL AUTO_INCREMENT,
  `customer_code` int(4) NOT NULL,
  `aankoopdatum` date DEFAULT NULL,
  CONSTRAINT pk_aankopen PRIMARY KEY (`aankoopnummer`)
...
INSERT INTO `aankopen` (`aankoopnummer`, `customer_code`, `aankoopdatum`) VALUES
(1, 100, '2024-08-01'),
(2, 101, '2024-08-05'),
(3, 100, '2024-08-08'),
(4, 100, '2024-08-15'),
...
CREATE TABLE IF NOT EXISTS `bestel_dnum` (
  `bestelnummer` varchar(4) NOT NULL,
  `supplier_code` varchar(3) NOT NULL,
  `besteldatum` int(6) NOT NULL,
  `delivery_date` int(6) NOT NULL,
...
INSERT INTO `bestel_dnum` (`bestelnummer`, `supplier_code`, `besteldatum`, `delivery_date`, `amount`) VALUES
('1', '1', 230130, 230214, 1000.00),
('2', '1', 230208, 230314, 1000.00),
('3', '1', 230330, 230414, 1000.00),
('4', '1', 230430, 230514, 1000.00),
('5', '1', 230530, 230614, 1000.00),
('6', '1', 230630, 230714, 1000.00),
('7', '1', 230730, 230814, 1000.00);
...
CREATE TABLE IF NOT EXISTS `orders` (
  `bestelnummer` int(4) NOT NULL AUTO_INCREMENT,
...
INSERT INTO `orders` (`bestelnummer`, `supplier_code`, `besteldatum`, `delivery_date`, `amount`, `status`) VALUES
...
CREATE TABLE IF NOT EXISTS `bestelregels` (
  `bestelnummer` int(4) NOT NULL,
  `article_code` int(4) NOT NULL,
  `amount` int(4) DEFAULT NULL,
  `order_price` decimal(4,2) DEFAULT NULL,
  PRIMARY KEY (`bestelnummer`,`article_code`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `bestelregels` (`bestelnummer`, `article_code`, `amount`, `order_price`) VALUES
(121, 314, 150, 0.45),
(121, 365, 150, 0.95),
(121, 422, 25, 2.25),
(121, 311, 50, 1.65),
...
INSERT INTO `boeking` (`boekstuk`, `boekdatum`, `amount`, `kllv_code`, `status`) VALUES
...
CREATE TABLE IF NOT EXISTS `boekregel` (
  ...
  `volgnummer`    int(2) NOT NULL,
  `amount`   decimal(9,2) NOT NULL,
  PRIMARY KEY (`boekstuk`,`volgnummer`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `boekregel` (`boekstuk`, `volgnummer`, `amount`) VALUES
(2, 1, 602.50),
(3, 1, 33.75),
(3, 2, 83.75),
(4, 1, 157.00),
...
CREATE TABLE IF NOT EXISTS `bestelregels` (
  ...
  `supplier_code` int(4) DEFAULT NULL,
  `order_price` decimal(4,2) DEFAULT NULL,
...
--
-- Gegevens worden uitgevoerd voor tabel `vat`
--
...
CREATE TABLE IF NOT EXISTS `factuur` (
  `factuurnummer` int(6) NOT NULL AUTO_INCREMENT,
  `factuurdatum` date NOT NULL,
  `status` char(1) DEFAULT NULL,
  `boekstuk` int(6) DEFAULT NULL,
  `volgnummer` int(2) DEFAULT NULL,
...
INSERT INTO `factuur` (`factuurnummer`, `factuurdatum`, `status`, `boekstuk`, `volgnummer`) VALUES
(1, '2024-08-07', 'A', 17, 1),
(2, '2024-08-10', 'A', 21, 1),
(3, '2024-08-15', 'A', 18, 1),
(4, '2024-08-19', 'A', 18, 2),
...
CREATE TABLE IF NOT EXISTS `goed_ontvangst` (
  `bestelnummer` int(4) NOT NULL,
  `article_code` int(4) NOT NULL,
  `ontvangst_datum` date NOT NULL,
  `ontvangst_aantal` int(4) NOT NULL,
  `status` char(1) NOT NULL,
  `boekstuk` int(6) DEFAULT NULL,
  `volgnummer` int(2) DEFAULT NULL,
  PRIMARY KEY (`bestelnummer`,`article_code`,`ontvangst_datum`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `goed_ontvangst` (`bestelnummer`, `article_code`, `ontvangst_datum`, `ontvangst_aantal`, `status`, `boekstuk`, `volgnummer`) VALUES
...
INSERT INTO `customers` (`customer_code`, `customer_name`, `customer_address`, `customer_postal_code`, `customer_phone`, `customer_city`, `customer_status`, `credit_limit`, `balance`) VALUES
(100, 'QA voetbalartikelen', 'Voetbalstraat 1', '2492 VJ', '012-3456789', 'Den Haag', 'A', 1000.00, 0.00),
(101, 'QA tennisartikelen', 'Tennisstraat 1', '3078 ZD', '032-1987654', 'Rotterdam', 'A', 1000.00, 0.00),
(102, 'QA algemene sport_articles', 'Sportlaan 1', '9728 jt', '045-6123789', 'Groningen', 'A', 1000.00, 0.00),
(103, 'QA fitness articles', 'Sportstraat 1', '1076 TW', '089-7654321', 'Amsterdam', 'A', 1000.00, 0.00);
...
INSERT INTO `suppliers` (`supplier_code`, `supplier_name`, `address`, `residence`) VALUES
(4, 'SPORTWINKEL HOVENIER G.H.', 'SPORTLAAN 50', 'ATHLETICAM'),
(9, 'BAUMGARTEN RACING TEAM', 'VELOCESTRAAT 13', 'CYCLANDIA'),
(11, 'STRUIK sport BV.', 'BASKETWEG 1', 'DUNKCITY'),
(13, 'SPITMAN sport EN ZN.', 'ATLETIEKLAAN 9', 'RUNNERSVILLE'),
(14, 'DEZAAIER LEISURE J.A.', 'RECREATIEWEG 101', 'SPORTHAVEN'),
(19, 'MOOIWEER FITNESS FA.', 'KRAFTSTRAAT 24', 'FITVILLAGE'),
(20, 'BLOEM LEISURE Z.H.W.', 'SPORTPARKLINNEAUSHOF 17', 'HILLEGYM'),
(21, 'TRA sport A.', 'FITNESSPLEKSTRAAT 10', 'LISSEREN'),
(22, 'ERICA sport BV.', 'BALWEG 87', 'HEEMFIT'),
(34, 'DE GROENE KAS FITNESS BV.', 'SPORTGLASWEG 1', 'FITLAND'),
(35, 'FLORA sport BV.', 'OEFENSTRAAT 76', 'AALSPORT');
...
CREATE TABLE IF NOT EXISTS `delivery` (
  ...
  `amount` int(6) NOT NULL,
  `factuurnummer` int(6) DEFAULT NULL,
  PRIMARY KEY (`aankoopnummer`,`article_code`,`delivery_date`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `delivery` (`aankoopnummer`, `article_code`, `delivery_date`, `amount`, `factuurnummer`) VALUES
(1, 1, '2024-08-08', 23, 1),
(1, 2, '2024-08-08', 2, 1),
...
CREATE TABLE IF NOT EXISTS `offertes` (
  `supplier_code` int(4) NOT NULL,
  `article_code` int(4) NOT NULL,
  `artcode_supplier` varchar(5) DEFAULT NULL,
  `levertijd` int(11) DEFAULT NULL,
  `offerteprijs` decimal(4,2) DEFAULT NULL,
...
INSERT INTO `offertes` (`supplier_code`, `article_code`, `artcode_supplier`, `levertijd`, `offerteprijs`) VALUES
...
CREATE TABLE `sport_articles` (
  ...
  `voorraad_min` int(11) NOT NULL DEFAULT 0,
...
INSERT INTO `sport_articles` (`article_code`, `article_name`, `category`, `size`, `color`, `price`, `voorraad_aantal`, `voorraad_min`, `VAT_type`) VALUES
...
--
-- Indexes for table `sport_articles`
--
...
--
-- AUTO_INCREMENT for table `sport_articles`
--
ALTER TABLE `sport_articles`
  MODIFY `article_code` int(4) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=500;
COMMIT;

alter table aankoop 
add	Constraint 	fk_aankoop_klant	foreign key (customer_code) references customers (customer_code),
add	Constraint 	fk_aankoop_sport_articles	foreign key (article_code) references sport_articles(article_code),
add	Constraint 	ck_aankoop_aantal	check (amount > 0 );

ALTER TABLE aankopen
add CONSTRAINT fk_aankoop_klant    FOREIGN KEY (customer_code) REFERENCES customers(customer_code);

alter table aankoopregel
add	Constraint 	fk_aankr_aank foreign key	(aankoopnummer) references aankopen(aankoopnummer),
add	Constraint 	fk_aankr_pla  foreign key	(article_code) references sport_articles(article_code);

alter table offertes
add Constraint 	fk_off_lev	  foreign key	(supplier_code) references suppliers(supplier_code),
add	Constraint 	fk_off_art	  foreign key	(article_code) references sport_articles(article_code);

alter table orders
add	Constraint 	fk_best_sup	  foreign key	(supplier_code) references suppliers(supplier_code);

alter table bestelregels
add	Constraint 	fk_breg_best  foreign key	(bestelnummer) references orders(bestelnummer),
add	Constraint 	fk_breg_art	  foreign key	(article_code) references sport_articles(article_code);
...
add	Constraint 	fk_gontv_breg	foreign key	(bestelnummer,article_code) references bestelregels(bestelnummer,article_code),
add	Constraint 	fk_gontv_boekr	foreign key	(boekstuk,volgnummer) references boekregel(boekstuk,volgnummer;

alter table factuur
add	Constraint 	fk_fact_boekr	foreign key	(boekstuk,volgnummer) references boekregel(boekstuk,volgnummer);

alter table delivery
add	Constraint 	fk_lever_aankr	foreign key	(aankoopnummer,article_code) references aankoopregel(aankoopnummer,article_code),
add	Constraint 	fk_lever_fact	foreign key (factnummer) references factuur(factnummer);
...
BEGIN
     SET NEW.volgnummer = (
        SELECT IFNULL(MAX(volgnummer), 0) + 1
        FROM boekregel
